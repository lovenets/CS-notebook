...
（5）当程序运行时，如果遇到引用空指针、下标越界或显式调用`panic`函数等情况，会先按照`defer`的后入先出的顺序，==只执行当前 goroutine 最后一个 defer==，最后才会执行`panic`。
...
func main() {
	...
	res := doSomethingWrong()
	fmt.Println(res)
}

func doSomethingWrong() (res bool) {
	defer func() {
		res = true
		log.Fatal("inner 1: ", res)
	}()
	defer func() {
		res = true
		log.Fatal("inner 2: ", res)
	}()
	a := []int{1, 2, 3}
	fmt.Println(a[3])
	return
